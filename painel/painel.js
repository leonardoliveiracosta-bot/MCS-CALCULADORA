(() => {
  ...
  async function sendConversation(parsed, sourceKind, sourceFilename, sourceSha, contact) {
    if (!parsed.supported) { $('import-status').textContent = `${sourceFilename}: formato não suportado — revisão, sem inserir mensagens.`; return { pending: true, inserted: 0 }; }
  ...
  async function importFiles(files) {
    ...
    for (const file of files) { $('import-status').textContent = `Lendo ${file.name}…`; const sourceSha = await sha256(await file.arrayBuffer()); for (const item of await extract(file)) { const result = await sendConversation(parseWhatsApp(item.text, item.name, $('mcs-name').value), file.name.toLowerCase().endsWith('.zip') ? 'WHATSAPP_ZIP' : 'WHATSAPP_TXT', file.name, sourceSha); inserted += result.inserted; pending = pending || result.pending; } }
  ...
  function renderQueue(chats) {
    const root = $('entry-queue'); root.replaceChildren(); if (!chats.length) { const p = document.createElement('p'); p.className = 'muted'; p.textContent = 'Nenhuma conversa pendente.'; root.append(p); return; }
  ...
  async function loadQueue() { const data = await request('/api/panel/entry'); contacts = data.contacts || []; const select = $('sms-contact'); const old = select.value; select.replaceChildren(new Option('Novo contato', 'new')); contacts.forEach((contact) => select.add(new Option(contact.display_name || 'Sem nome', contact.id))); if ([...select.options].some((option) => option.value === old)) select.value = old; renderQueue(data.chats || []); }
