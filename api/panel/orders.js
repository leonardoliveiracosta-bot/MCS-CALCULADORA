  ...
  try {
    ...
    const calculator = consolidateCalcRuns(calcRuns, links).map((item) => {
      ...
      return {
        ...
        status: !item.link ? 'SEM CONTATO' : latest && latest.direction === 'CUSTOMER' ? 'SEM RESPOSTA' : 'RESPONDIDO',
    ...
    const direct = data.journeys.filter((item) => ['WHATSAPP_DIRECT', 'SMS_DIRECT'].includes(item.source)).map((item) => {
      ...
      return {
        ...
        logicalMode: item.budget_cents ? 'CARRO' : 'VALOR',
    ...
    const items = calculator.concat(direct).filter((item) => {
    ...
    return send(res, 200, { environment: ctx.environment, filter, items, linkTargets, meta });
  } catch (_) {
