  ...
  try {
    const [journeys, contacts, refs, checklist, evidence, divergences, meta] = await Promise.all([
      allRows(ctx, 'journeys', {
        select: 'id,contact_id,reference_code,stage,status,vehicle_text,budget_cents,payment_text,customer_deadline_at,customer_deadline_text,qualified_at,closed_reason,updated_at',
