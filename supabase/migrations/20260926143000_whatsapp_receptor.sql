create table if not exists public.whatsapp_raw_events (
 ...
 status text not null default 'PENDING' check (status in ('PENDING','PROCESSING','DONE','ERROR','IGNORED')),
 error_code text,
...
create index if not exists whatsapp_raw_events_queue_idx on public.whatsapp_raw_events(environment,status,received_at);
alter table public.whatsapp_raw_events enable row level security;
alter table public.whatsapp_raw_events force row level security;
revoke all on public.whatsapp_raw_events from anon,authenticated;
grant select,insert,update on public.whatsapp_raw_events to service_role;

create table if not exists public.whatsapp_message_ids (
 environment public.panel_environment not null,
 wa_message_id text not null,
 message_id uuid not null references public.messages(id),
 raw_event_id uuid not null references public.whatsapp_raw_events(id),
 primary key(environment,wa_message_id)
);
alter table public.whatsapp_message_ids enable row level security;
alter table public.whatsapp_message_ids force row level security;
revoke all on public.whatsapp_message_ids from anon,authenticated;
grant select,insert on public.whatsapp_message_ids to service_role;

create table if not exists public.whatsapp_link_suggestions (
 id uuid primary key default gen_random_uuid(),
 environment public.panel_environment not null,
 source_contact_id uuid not null references public.contacts(id),
 source_journey_id uuid not null references public.journeys(id),
 target_contact_id uuid not null references public.contacts(id),
 target_journey_id uuid not null references public.journeys(id),
 phone_e164 text not null,
 status text not null default 'PENDING' check (status in ('PENDING','LINKED','REJECTED')),
 created_at timestamptz not null default now(),
 resolved_at timestamptz,
 resolved_by uuid,
 unique(environment,source_journey_id,target_journey_id)
);
alter table public.whatsapp_link_suggestions enable row level security;
alter table public.whatsapp_link_suggestions force row level security;
revoke all on public.whatsapp_link_suggestions from anon,authenticated;
grant select,insert,update on public.whatsapp_link_suggestions to service_role;
...
declare phone text := p_item->>'phone'; wid text := p_item->>'messageId'; nm text := left(coalesce(nullif(trim(p_item->>'name'),''),phone),160);
  ...
  candidate record; new_contact boolean := false; norm text; other_count integer;
begin
 ...
 if other_count>1 then raise exception 'PHONE_AMBIGUOUS'; end if;
 ...
 if not exists(select 1 from public.contact_phones cp where cp.environment=p_environment and cp.contact_id=contact_id and cp.phone_e164=phone and cp.retired_at is null) then
   insert into public.contact_phones(environment,contact_id,phone_raw,phone_e164,is_current,created_at)
     values(p_environment,contact_id,phone,phone,true,now());
 end if;
 ...
 if journey_id is null then
   insert into public.journeys(environment,contact_id,source,stage,status,criteria_json,created_at,updated_at)
     values(p_environment,contact_id,'WHATSAPP_DIRECT','NOVO','ATIVO','{}'::jsonb,stamp,now()) returning id into journey_id;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare s public.whatsapp_link_suggestions%rowtype; m record; existing_id uuid; linked_count integer:=0; skipped_count integer:=0;
begin
 select * into s from public.whatsapp_link_suggestions where id=p_id and environment=p_environment for update;
 if not found or s.status<>'PENDING' then raise exception 'SUGGESTION_NOT_PENDING'; end if;
 if not exists(select 1 from public.panel_users where id=p_actor and environment=p_environment and active=true) then raise exception 'PANEL_ACCESS_DENIED'; end if;
 if p_link then
   perform pg_advisory_xact_lock(hashtext(p_environment::text||':'||s.phone_e164));
   if not exists(select 1 from public.journeys where id=s.target_journey_id and environment=p_environment and contact_id=s.target_contact_id) then raise exception 'TARGET_INVALID'; end if;
   update public.contact_phones set contact_id=s.target_contact_id where environment=p_environment and contact_id=s.source_contact_id and phone_e164=s.phone_e164;
   ...
   for m in select q.message_id, x.body_normalized,x.direction,x.occurred_at_utc from public.message_journeys q
     ...
     select x.id into existing_id from public.message_journeys q join public.messages x on x.id=q.message_id
       where q.environment=p_environment and q.journey_id=s.target_journey_id and x.id<>m.message_id and
         x.direction=m.direction and x.body_normalized=m.body_normalized and
         date_trunc('minute',x.occurred_at_utc)=date_trunc('minute',m.occurred_at_utc) limit 1;
