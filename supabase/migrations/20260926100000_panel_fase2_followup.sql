-- Complete Production identities without removing or changing older duplicate units.
with ranked as (
 select id, environment, journey_id,
   coalesce(vehicle_identity, case
     ...
     else 'CAR:' || lower(concat_ws('|',details_json->>'year',details_json->>'make',details_json->>'model',details_json->>'miles',details_json->>'saleDate')) end) as identity,
   row_number() over (partition by environment, journey_id, coalesce(vehicle_identity, case
     when details_json is null then null
     when nullif(upper(trim(details_json->>'vin')),'') is not null then 'VIN:' || upper(trim(details_json->>'vin'))
     ...
     order by (vehicle_identity is not null) desc, presented_at, id) as seq
 from public.units where environment='production'
)
update public.units u set vehicle_identity=r.identity from ranked r
 where u.id=r.id and u.environment='production' and u.vehicle_identity is null and r.identity is not null and r.seq=1;
